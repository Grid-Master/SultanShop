...
const FilterAside:FC<IFilterAsideProp> = ({filterHandler, activeFilter}) => {
    ...
    const [manufacturersChecked, setManufacturerChecked] = useState<boolean[]>(
        manufacturers.map(() => false )
    )
    ...
    const filterManufacturerHandler = (e:React.ChangeEvent<HTMLInputElement>) => {
        ...
        let checked = manufacturersChecked.map(() => {
            for(let el of manufacturersChecked) {
                console.log(manufacturersChecked.indexOf(el))
                if (manufacturersChecked.indexOf(el).toString() === e.target.id) {
    ...
    return (
        <aside className='filter'>
            ...
            <div className='filter__manufacturer'>
                ...
                <ul id = 'manufacturers'>
                    {manufacturersChecboxMore ? manufacturers.map((manufacturer, id) => (
                        <li key={manufacturer}>
                            <input 
                                type='checkbox' 
                                id={`${id}`} 
                                ...
                                name={manufacturer} 
                                checked={manufacturersChecked[id]} 
                            />
                            <label htmlFor={`${id}`}>{manufacturer}<span>({manufacturerCount(manufacturer)})</span></label>
                        </li>
                    ))
                    :
                    manufacturers.slice(0, 4).map((manufacturer, id) => (
                        <li key={manufacturer}>
                            <input 
                                type='checkbox' 
                                id={`${id}`} 
                ...
                <p onClick={() => setManufacturerCheckboxMore(!manufacturersChecboxMore)}>
                    ...
                    <img className={manufacturersChecboxMore ? 'polygon polygon-rotated' : 'polygon'} src={polygon} alt='polygon' />
                </p>
            </div>
