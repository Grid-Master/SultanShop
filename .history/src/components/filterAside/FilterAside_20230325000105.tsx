...
const FilterAside:FC<IFilterAsideProp> = ({filterHandler}) => {
    ...
    const [priceRangeMax, setPriceRangeMax] = useState<string>('50')
    ...
    const filterPriceRangeHandlerMin = (e:React.ChangeEvent<HTMLInputElement>) => {
        ...
        } else if(priceRangeMin.length < (priceRangeMin + e.target.value).length) {
            setPriceRangeMin(priceRangeMin + e.target.value)
        } else {
           setPriceRangeMin(priceRangeMin.slice(0, -1))
        }
        dispatch(filterPriceRangeGoods([priceRangeMin, priceRangeMax]))
    }
