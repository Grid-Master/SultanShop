...
import { filterGoods, reloadGoods, sortGoodsByPrice, sortGoodsByTitle } from '../../store/catalogGoodsSlice';
...
const Catalog: FC = () => {
    ...
    useEffect(() => {
        currentGoods = catalogGoods.slice(indexOfFirstGood, indexOfLastGood);
    ...
    const sortHandler = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        switch(e.target.value) {
            case 'price+': dispatch(sortGoodsByPrice('+'))
            break
            case 'price-': dispatch(sortGoodsByPrice('-'))
            break
            case 'title+': dispatch(sortGoodsByTitle('+'))
            break
            case 'title-': dispatch(sortGoodsByTitle('-'))
            break
        }
        ...
        <div className='catalog'>
            <div className='container'>
                ...
                <ul className='catalog__hygienelist'>
                    <li className={activeFilter === 'body' ? 'activeFilter' : ''} onClick={() =>filterHandler('body')}>Уход за телом</li>
                    <li className={activeFilter === 'hands' ? 'activeFilter' : ''} onClick={() =>filterHandler('hands')}>Уход за руками</li>
                ...
                <div className='catalog__container-goods'>
                    ...
                    <div>
                        <div className='catalog__container-goods-grid'>
                            {currentGoods ? currentGoods.map((good: any) => <CatalogGood good={good} key={ good.id} />) : ''}
                        </div>
                        {goods && <ReactPaginate
