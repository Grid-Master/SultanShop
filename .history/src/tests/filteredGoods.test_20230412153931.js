...
describe('фильтр товаров', () => {
  ...
  it('фильтр товаров по типу ухода за телом', () => {
    const filteredGoods = catalogGoods.filter((good) => good.careType.includes('body'));
    expect(catalogGoodsReducer({ goods: catalogGoods }, filterGoods('body'))).toEqual({
      goods: filteredGoods,
    });
  });
});
