  ...
  reducers: {
    ...
    sortGoodsByTitle(state, action: PayloadAction<string>) {
      if (state.goods) {
        switch (action.payload) {
          case '+':
            state.goods = state.goods.slice().sort((a, b) => a.title.localeCompare(b.title));
            break;
          case '-':
    ...
    reloadGoods(state, action: PayloadAction<IGood[]>) {
...
export const editCareTypeFromAdmin = (data: string[]) => (dispatch: any) => {
  dispatch(adminActions.editTypeCare(data));
  dispatch(editTypeCare(data));
};
